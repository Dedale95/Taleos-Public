...
Lazard — Job Scraper
=====================
Source : Oracle Cloud HCM (portal carrières Lazard)
  Host       : icbpjb.fa.ocs.oraclecloud.com
  Site       : CX_1 (LazardProfessionalCareers)
...
Structure API :
  listing : Id, Title, PostedDate, PrimaryLocation, PrimaryLocationCountry,
            JobFamily, JobFunction, WorkerType, ContractType, JobType
  détail  : ExternalDescriptionStr, ExternalResponsibilitiesStr,
            ExternalQualificationsStr, StudyLevel, ContractType, JobLevel

Delta scraping : oracle_id (Id) comme clé unique
                 → listing toujours fetché
                 → détail uniquement pour nouvelles offres (37 offres total)
"""
import json
import logging
import re
...
logging.basicConfig(
    ...
    handlers=[logging.StreamHandler(sys.stdout),
              logging.FileHandler("lazard_scraper.log", encoding="utf-8")],
)
logger = logging.getLogger(__name__)

# ─── Config ──────────────────────────────────────────────────────────────────
ORACLE_HOST   = "https://icbpjb.fa.ocs.oraclecloud.com"
SITE_NUMBER   = "CX_1"
...
DB_PATH       = Path(__file__).parent / "lazard_jobs.db"
PAGE_SIZE     = 100
REQUEST_PAUSE = 0.4
...
# ─── Mapping ContractType Oracle → Taleos ─────────────────────────────────────
_CONTRACT_PATTERNS = [
...
# Oracle JobLevel → Taleos experience
LEVEL_MAP = {
    "Senior":     "6 - 10 ans",
    "Mid":        "3 - 5 ans",
    "Junior":     "0 - 2 ans",
...
ISO2_COUNTRY = {
    ...
    "ZA": "Afrique du Sud", "MX": "Mexique", "AR": "Argentine",
    "SA": "Arabie saoudite", "QA": "Qatar", "KW": "Koweït",
}


def _detect_contract(title: str, oracle_contract: Optional[str]) -> str:
    for pat, ctype in _CONTRACT_PATTERNS:
        if pat.search(title or ""):
            return ctype
    # Mapper les valeurs Oracle
    c = (oracle_contract or "").upper()
    if "INTERN" in c:    return "Stage"
    if "FIXED" in c:     return "CDD"
    if "PART" in c:      return "Temps partiel"
    return "CDI"


def _detect_level(title: str, job_level: Optional[str]) -> str:
...
def init_db(conn: sqlite3.Connection):
    ...
    CREATE TABLE IF NOT EXISTS jobs (
        ...
        company_name     TEXT DEFAULT 'Lazard',
...
def get_existing_oracle_ids(conn: sqlite3.Connection) -> set:
...
def fetch_all_listings(session: requests.Session) -> list:
    """Récupère toutes les offres en une requête (37 offres → PAGE_SIZE suffit)."""
    params = {
        "onlyData": "true",
        "expand": "requisitionList.secondaryLocations",
        "finder": f"findReqs;siteNumber={SITE_NUMBER},limit={PAGE_SIZE},offset=0",
    }
    all_reqs = []
    offset = 0
    while True:
        params["finder"] = f"findReqs;siteNumber={SITE_NUMBER},limit={PAGE_SIZE},offset={offset}"
...
def parse_listing_req(req: dict) -> dict:
    ...
    # Parser ville depuis PrimaryLocation "City, Country"
    city = location.split(",")[0].strip() if "," in location else location

    contract_raw = req.get("ContractType") or req.get("JobType")
    contract     = _detect_contract(title, contract_raw)
...
def enrich_with_detail(row: dict, detail: dict) -> dict:
    ...
    for key in ("ExternalDescriptionStr", "ExternalResponsibilitiesStr", "ExternalQualificationsStr"):
        text = _html_to_text(detail.get(key, "") or "")
        if text:
            parts.append(text)
    description = "\n\n".join(parts)
    ...
    # Affiner le contrat depuis le détail
    if not row.get("contract_type") or row["contract_type"] == "CDI":
        detail_contract = detail.get("ContractType") or detail.get("RequisitionType") or ""
        row["contract_type"] = _detect_contract(row["job_title"], detail_contract)
...
def main():
    t0 = time.time()
    logger.info("=" * 60)
    logger.info("Lazard Scraper — démarrage")

    conn = sqlite3.connect(DB_PATH)
    conn.row_factory = sqlite3.Row
    init_db(conn)

    existing_ids = get_existing_oracle_ids(conn)
    ...
    logger.info(f"   → {new_count} nouvelles offres, {len(all_reqs)-new_count} déjà connues")

    # ── Phase 2 : détails (toutes les offres — seulement 37) ────────────
    ...
    enriched = 0
    for oracle_id in ids_to_enrich:
        if oracle_id not in rows:
            continue
        detail = fetch_detail(session, oracle_id)
        if detail:
            rows[oracle_id] = enrich_with_detail(rows[oracle_id], detail)
            enriched += 1
        time.sleep(REQUEST_PAUSE)
    ...
    logger.info("=" * 60)
    logger.info(f"✅ Lazard — {total_live} offres Live en base")
